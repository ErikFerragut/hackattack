...
\begin{abstract}
  ...
  and defense strategies. (example of one that's not very realistic.)
  However, the overwhelming majority of cyber game theory explores
  ...
  complex games are possible, but the game presented here is the most
  realistic cyber game we know of. While optimal strategies can be
  found for simpler games using min-max optimization or [whatever they
  do on stochastic Markov chains], our more complex game must be
  analyzed using the same kinds of analyses used to study other
  complex games, such as checkers and chess. We define a simple
  evaluation function and employ $k$-ply searches to create
  strategies. We show that such scenarios can be analyzed, and find
  that in cases of extreme uncertainty, it is often better to ignore
  one's opponent's possible moves.
\end{abstract}
...
\input{cyber-game-intro.tex}


\section{Background}
\input{cyber-game-background.tex}

\section{Description of HackAttack}
\input{cyber-game-hackattack.tex}

\section{Strategies Considered}
\input{cyber-game-kply.tex}
...
\section{Results}
\input{cyber-game-results.tex}

\section{Discussion}
\input{cyber-game-discussion.tex}

\section{Conclusions}
\input{cyber-game-conclusions.tex}
