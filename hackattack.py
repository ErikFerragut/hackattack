...
move_funcs = {'r':do_recon, 'c':do_clean, 'h':do_hack, 'b':do_backdoor,
              'p':do_patch, 'd':do_ddos}
...
# or
players_names = [ "Player {}".format(i) for i in xrange(num_players) ]
...
while True:
    ...
    if sum(players_own[player].values()) == 0:
        print "{} is out".format(players_names[player])
        player = (player + 1) % num_players
        continue
    elif all( [ sum(players_own[p].values()) == 0 for p in xrange(num_players)
                if p != player ] ):
        print "You won!"
        break
    ...
    raw_input("\n"*100 + "Ready {}? ".format(players_names[player]))
    
    if len(news[player]) == 0:
        print "No news to report on round {}".format(game_round)
    else:
        print "\n   ".join(["ROUND {} NEWS!".format(game_round)] + news[player])
        news[player] = []

	## show them what they have
    print "Your access:"
    for k,v in players_own[player].iteritems():
        if v > 0:
            print "{} account{} on machine {}, which runs the {} OS".format(
                v, 's' if v > 1 else '', k, board_os[k])
    ## check for a new exploit
    if random.random() <= 1./6:
        ne = random.choice(list(set(all_exploits).difference(players_expl[player])))
        players_expl[player].append( ne )
    ...
    print "\nTraced players: {}".format( "None" if len(players_traced[player]) == 0
                                         else " ".join(map(str, players_traced[player])) )
    ...
    while len(moves) < len(players_own[player].keys()):
#    for machine in sorted(players_own[player].keys()):
    ...
    for move in moves:
        ...
        if move['action'] in move_funcs:
            move_funcs[move['action']](move)
        else:
