...
class Game(object):
    ...
    def do_scan(self, move):
        ...
        for s in xrange(self.num_players):
            if s != player:
                if move['from'] in player.own[s] and player.own[s][move['from']] > 0:
                    num_removed = 10000
                    player.own[s][move['from']] -= num_removed
                    if player.own[s][move['from']] == 0:
                        player.own[s].pop(move['from'])
